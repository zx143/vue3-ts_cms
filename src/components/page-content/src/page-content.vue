 ...
 * @LastEditTime: 2021-10-16 20:37:23
-->
<template>
  <div class="page-content">
    ...
    >
      <template v-slot:headerHandle>
        <el-button v-if="headerHandleBtnText && isCreate" type="primary">{{
          headerHandleBtnText
        }}</el-button>
      </template>
      ...
      <template #operation="scope">
        <div class="operation-btns">
          <el-button v-if="isUpdate" size="mini" type="text" icon="el-icon-edit">编辑</el-button>
          <el-button
            v-if="isDelete"
...
export default defineComponent({
  ...
  setup(props) {
    ...
    /* 发送请求获取数据 */
    // 向table组件双向绑定页码参数
    const pageInfo = ref({ currentPage: 0, size: 10 })
    // 监听页码参数发生改变 重新发起请求
    ...
    const getPageData = (searchInfo: any = {}) => {
      ...
      const pageListParams: IPageListParams = {
        ...
        queryInfo: {
          offset: pageInfo.value.currentPage * pageInfo.value.size,
